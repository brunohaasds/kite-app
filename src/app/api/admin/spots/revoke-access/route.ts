...
import { revokeAccessSchema } from "@/domain/global-spots/schema";
import { revokeAccess } from "@/domain/global-spots/repo";
...
export async function POST(request: Request) {
  ...
  if (!isSuperAdmin && spot.owner_organization_id !== orgId) {
    return NextResponse.json(
      { error: "Apenas o dono do spot pode revogar acesso" },
      { status: 403 },
    );
  }

  try {
    await revokeAccess(parsed.data.global_spot_id, parsed.data.organization_id);
