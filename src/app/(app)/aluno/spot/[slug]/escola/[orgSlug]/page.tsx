...
export default async function AlunoSpotSchoolPage({ params }: Props) {
  const { slug, orgSlug } = await params;
    ...
    <SchoolLandingView
      ...
      topSlot={
        <div className="flex flex-wrap items-center gap-1 border-b bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
          <Link href="/aluno/spots" className="hover:text-foreground">
            Spots
          </Link>
          <ChevronRight className="h-3.5 w-3.5 shrink-0" />
          <Link href={`/aluno/spot/${spot.slug}`} className="hover:text-foreground">
            {spot.name}
          </Link>
          <ChevronRight className="h-3.5 w-3.5 shrink-0" />
